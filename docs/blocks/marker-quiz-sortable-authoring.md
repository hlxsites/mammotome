...
The block should contain a link to your JSON data file:

```html
<div class="marker-quiz">
  <a href="/path/to/survey-data.json">Survey Data</a>
</div>
```

Or you can provide the Marketo Form ID in section metadata:
- `marketoformid` or `marketo-form-id`: The Marketo form ID for the contact form

## Authoring Tool Tips

When working in your authoring/spreadsheet tool:

1. **Sortable Column**: Set to `TRUE` or `true` (case may vary by tool)
2. **RankScores Column**: Paste the entire JSON string as a single value
   - Ensure no line breaks in the JSON
   - Double-check all quotes are properly escaped
   - Validate JSON syntax before saving
3. **QuestionType**: Leave empty or set to `single` for sortable questions
4. **Options**: Ensure `QuestionId` matches the question `Id`

## Validation Checklist

- [ ] Question has `Sortable` set to `"true"`, `"TRUE"`, or `true`
- [ ] `QuestionType` is empty or set to `"single"` (or omitted, as it defaults to `single`)
- [ ] `RankScores` is a valid JSON string (no syntax errors)
- [ ] `RankScores` contains keys for all possible ranks (`rank_1`, `rank_2`, etc.)
- [ ] Number of rank keys matches the number of options (e.g., 4 options = rank_1 through rank_4)
- [ ] Product IDs in `RankScores` match actual product `Id` values exactly (case-sensitive)
- [ ] Options have `QuestionId` matching the question `Id`
- [ ] All scores in `RankScores` are valid numbers

## Technical Notes

- `RankScores` is parsed using `JSON.parse()` in the code
- If `RankScores` parsing fails, it defaults to an empty object `{}` and scoring may not work correctly
- The sorted order is stored in `sortedOptions[questionId]` as an array of option text strings
- Rank positions are 1-indexed (`rank_1`, `rank_2`, etc.) but arrays are 0-indexed
- The `sortType` and `sortWeight` fields are not used for sortable questions (they apply to non-sortable questions)
