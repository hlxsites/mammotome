  ...
  window.MktoForms2.whenReady((form) => {
    ...
    const isCustomValid = (native, currentStep) => {
      ...
      const currentUnfilled = userConfig.requiredFields.filter((fieldDesc) => {
        ...
        if (fieldDesc.name === 'Email' && isInCurrentStep) {
          ...
          if (!value) {
            if (fieldDesc.refEl) fieldDesc.refEl.setCustomValidity('This field is required.');
            return true;
          }
          if (!valid) {
            if (fieldDesc.refEl) fieldDesc.refEl.setCustomValidity('Please enter a valid email address.');
            return true;
          }
          return false;
        }
        return isInCurrentStep && (!value || (fieldDesc.refEl.type === 'checkbox' && value === 'no'));
      });
      if (currentUnfilled.length) {
        const field = currentUnfilled[0];
        const message = field.refEl?.validationMessage || field.message || 'This field is required.';
        ...
        selectedInputs.forEach((selectedInput) => {
          ...
          if (value) {
            value.split(';').forEach((letter) => {
              ...
              if (resultName) resultsMap[resultName] += resultsMap[resultName] + 1;
            });
          }
        });
        const topResult = Object.entries(resultsMap).sort((a, b) => b[1] - a[1])[0][0];
        console.log('Pending top result:', topResult);
      });
    });

    arrayify(fieldRows).forEach((row, rowIdx) => {
      const rowPos = { isFirst: rowIdx === 0, isLast: rowIdx === fieldRows.length - 1 };
      ...
      if (nextEnabled) navButtons.next = navButtonRow.querySelector(buttonStor);
      if (prevEnabled) {
      ...
      if (nextEnabled) row.parentNode.insertBefore(navButtonRow, newRowAxis);
      if (prevEnabled) newButtonAxis.parentNode.insertBefore(navButtons.prev, newButtonAxis);
    ...
    form.onValidate(() => {
      ...
      if (isValid) {
        ...
        // UPDATE: NEXT STEPS FIELD TO PRODUCT RESULT
        const topResult = Object.entries(resultsMap).sort((a, b) => b[1] - a[1])[0][0];
        form.addHiddenFields({
          'Next Steps': topResult,
        });
