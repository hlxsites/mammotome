...
const FORM_SUBMIT_ENDPOINT =
  "https://franklin-submit-wrapper.mammotome.workers.dev";
...
async function fetchSurveyData(url) {
  try {
    ...
    if (!resp.ok) {
      // eslint-disable-next-line no-console
      console.error(
        `Failed to fetch survey data: ${resp.status} ${resp.statusText}`
      );
      return null;
    }
...
class ProductSurvey {
  ...
  async loadSurveyData() {
    const surveyLink = this.block.querySelector(
      'a[href$=".json"], a[href*=".json"]'
    );
    ...
    // eslint-disable-next-line no-console
    console.error(
      "No valid survey data found. Please provide a JSON file link in the block."
    );
  ...
  render() {
    ...
    if (previousAnswer) {
      if (currentQuestion.type === "multi") {
        const answerArray = Array.isArray(previousAnswer.answer)
          ...
          : [previousAnswer.answer];
        this.selectedOptions = currentQuestion.options.filter((opt) =>
          answerArray.some((ans) => {
    ...
    this.block.innerHTML = `
      ...
      currentQuestion.layout === "horizontal"
        ? "layout-horizontal"
        : "layout-vertical"
    }">
                ${currentQuestion.options
                  ...
                  ${
                    showOtherInput
                      ? `<input type="text" class="other-input" data-option-text="${option.text}" placeholder="Please specify..." value="${otherValue}" />
  ...
  renderStartScreen() {
    ...
    if (startBtn) {
      startBtn.addEventListener("click", () => {
        ...
        const surveyContainer = this.block.querySelector(
  ...
  selectOption(optionIndex) {
    ...
    if (isMulti) {
      ...
      const optionElement = this.block.querySelector(
        `.option[data-option-index="${optionIndex}"]`
      );
      if (optionElement) {
        ...
        if (existingIndex >= 0) {
          ...
          const existingLabel = optionElement.querySelector(
  ...
  updateNextButtonState() {
    ...
    if (isMulti) {
      ...
      const hasOtherWithoutText = this.selectedOptions.some(
        (opt) =>
          opt.isOther === true &&
          (!this.otherTexts[opt.text] ||
            this.otherTexts[opt.text].trim() === "")
      );
  ...
  async nextQuestion() {
    ...
    if (isMulti) {
      ...
      this.selectedOption?.isOther === true &&
  ...
  async showContactForm() {
    const marketoFormId =
      this.config.marketoformid || this.config["marketo-form-id"];
    ...
    if (marketoFormId) {
      ...
      embedMarketoForm(formWrapper, marketoFormId)
        .then((form) => {
          form.onSuccess((values) => {
            ...
            this.submitQuizData(contactData)
              .then(() => {
        ...
        .catch((error) => {
          ...
          formWrapper.innerHTML =
            '<p class="error">Unable to load contact form. Please try again later.</p>';
        });
    } else {
      // eslint-disable-next-line no-console
      console.warn(
        'No Marketo form ID provided. Please add "Marketo Form ID" to the block configuration.'
      );
      const formWrapper = this.block.querySelector("#marketo-form-wrapper");
      formWrapper.innerHTML =
        '<p class="error">Contact form not configured. Please contact the site administrator.</p>';
    }
