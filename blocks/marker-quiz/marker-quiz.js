...
class ProductSurvey {
  ...
  getScoreWeight(question, answerText) {
    if (!question.sortType || question.sortType === 'normal') {
      return question.sortWeight || 1;
    }

    if (question.sortType === 'priority') {
      return question.sortWeight || 2;
    }

    if (question.sortType === 'filter') {
      return answerText ? 1 : 0;
    }

    if (question.sortType === 'multiplier') {
      const option = question.options.find((opt) => opt.text === answerText);
      return option && option.multiplier ? option.multiplier : 1;
    }
