...
export default function decorate(block) {
  ...
  const baseNoticeId = '0e664032-8cf4-4719-94d8-fdd9685743b5';
  ...
  const initializeOneTrust = async () => {
    await OneTrust.NoticeApi.Initialized.then(async () => { // eslint-disable-line
      ...
      console.log('OneTrust CCPA Notice URL (attempting):', noticeUrl);

      try {
        await OneTrust.NoticeApi.LoadNotices([noticeUrl], false); // eslint-disable-line
        // eslint-disable-next-line no-console
        console.log('OneTrust CCPA Notice loaded successfully');
      } catch (error) {
        ...
        if (locale) {
          ...
          try {
            ...
            // eslint-disable-next-line no-console
            console.log('OneTrust CCPA Notice loaded successfully (English fallback)');
          } catch (fallbackError) {
