import { getMetadata, decorateIcons, getI18nOrDefault } from '../../scripts/lib-franklin.js';
...
async function searchInput(event) {
  ...
  if (value.length >= 3) {
    ...
    title.textContent = `${await getI18nOrDefault('navSearchResultsFor', 'Search Results for')}: ${value}`;
    aside.append(title);
    ...
    try {
      ...
      if (hits.length > 0) {
        ...
        const searchTitle = document.createElement('h3');
        searchTitle.classList.add('nav-search-title');
        searchTitle.textContent = await getI18nOrDefault('navSearchNoResult', 'No Result');
        aside.appendChild(searchTitle);
      }
    } catch (error) {
      const searchTitle = document.createElement('h3');
      searchTitle.classList.add('nav-search-title');
      searchTitle.textContent = await getI18nOrDefault('navSearchFailure', 'Search could not be completed at this time - please try again later.');
      aside.appendChild(searchTitle);
    }
    aside.insertAdjacentHTML('beforeend', '<div class="nav-search-result-title-divider"><span class="nav-search-result-title-divider-separator"/></div>');
  }
...
async function searchClick(event) {
  ...
  if (!input.active) {
    input.placeholder = await getI18nOrDefault('navSearchPlaceholder', 'What are you looking for?');
