...
const SHEET_TOKEN = 'mmt-quiz-2026-secure-token';

/**
 * Sends quiz results to Google Sheets via Apps Script web app.
...
class MarkerQuiz {
    ...
    buildSheetPayload() {
        ...
        const modalities = Array.isArray(modalitySel) ? modalitySel : (modalitySel != null ? [modalitySel] : []);
        ...
        return {
            ...
            patient_cases: (() => {
                ...
                const cases = Array.isArray(sel) ? sel : (sel != null ? [sel] : []);
    ...
    showResults() {
        ...
        this.block.querySelector('#lead-submit-btn')?.addEventListener('click', () => {
            ...
            sendToSheet({ ...this.buildSheetPayload(), name, email, facility });
        });
